...
const OUTPUT_PATH = path.join(DATA_DIR, "scalp-candles.json");
const SYMBOLS = [
    {
        symbol: "XAU/USD",
        key: "XAUUSD",
 ...
 */
const MAX_REQUESTS_PER_RUN = SYMBOLS.length;
 ...
 */
function getCandleTimeState(time, referenceTimeMs = Date.now()) {
    const candleTime = timestampToMs(time);
...
function isPossiblyOpenCandle(time, referenceTimeMs = Date.now()) {
    return (getCandleTimeState(time, referenceTimeMs) === "open");
}
function isClosedCandle(time, referenceTimeMs = Date.now()) {
    return (getCandleTimeState(time, referenceTimeMs) === "closed");
}
/*
 * Main output arrays are closed-candle-only. This helper remains defensive
 * for cached or externally supplied arrays.
 */
function countClosedCandles(rows, referenceTimeMs = Date.now()) {
    if (!Array.isArray(rows)) {
        return 0;
    }
    return rows.reduce((count, row) => isClosedCandle(row?.time, referenceTimeMs)
        ? count + 1
        : count, 0);
}
/* =====================================================================
...
function readJsonFile(filePath, fallback = null) {
    try {
        ...
        console.warn(`Could not safely read ${filePath}: ${error.message}`);
        return fallback;
    }
}
...
/* =====================================================================
   Candle Validation
   ===================================================================== */
function normalizeCandle(raw, referenceTimeMs = Date.now()) {
    if (!raw ||
        typeof raw !== "object") {
        return {
            candle: null,
    ...
    const safeReferenceTimeMs = Number.isFinite(referenceTimeMs)
        ? referenceTimeMs
        : Date.now();
    const time = normalizeTimestamp(raw.datetime ||
        raw.time);
    if (!time) {
        return {
            candle: null,
    ...
    const open = parsePrice(raw.open);
...
function normalizeCandles(values, referenceTimeMs = Date.now()) {
    ...
    for (const raw of values) {
        const result = normalizeCandle(raw, safeReferenceTimeMs);
        if (!result.candle) {
            rejectedCount++;
         ...
         */
        byTime.set(time, result.candle);
    }
    const rows = Array.from(byTime.values())
        .sort((a, b) => timestampToMs(a.time) -
    ...
    return {
        rows,
        quality: {
            receivedRows: values.length,
            validRows: rows.length,
            rejectedRows: rejectedCount,
            duplicateTimes: duplicateCount,
            rejectedReasons,
            firstTime,
            lastTime,
...
function getPreviousRows(previousOutput, key) {
     ...
     */
    const nestedRows = previousOutput
        ?.symbols?.[key]
        ?.candles;
...
/* =====================================================================
   ...
   ===================================================================== */
function createProviderError(symbol, payload) {
    ...
    const error = new Error(`Twelve Data error for ${symbol}: ${providerMessage}`);
    error.providerCode =
        providerCode;
...
async function fetchJsonOnce(url, timeoutMs = REQUEST_TIMEOUT_MS) {
    if (requestsMade >=
        ...
        throw new Error(`Request safety limit reached: maximum ` +
    ...
    try {
        ...
        if (!response.ok) {
            const providerMessage = payload &&
                ...
                ? payload.message
                : `HTTP ${response.status}`;
            const error = new Error(`Twelve Data request failed: ${providerMessage}`);
            error.httpStatus =
                response.status;
            error.providerPayload =
                payload;
        ...
        if (error &&
            error.name ===
                "AbortError") {
            throw new Error(`Twelve Data request timed out after ${timeoutMs}ms`);
        }
        throw error;
    }
    finally {
...
async function fetchCandles(symbol) {
    ...
    if (!payload ||
        typeof payload !== "object") {
        throw new Error(`Empty or invalid Twelve Data response for ${symbol}`);
    }
    /*
    ...
    if (payload.status === "error" ||
        payload.code != null) {
        throw createProviderError(symbol, payload);
    }
    ...
    return {
        ...
        providerMeta: {
            ...
            interval: payload.meta?.interval ||
                INTERVAL,
            currencyBase: payload.meta
                ?.currency_base ||
                null,
            currencyQuote: payload.meta
                ?.currency_quote ||
                null,
            exchange: payload.meta?.exchange ||
                null,
            exchangeTimezone: payload.meta
                ?.exchange_timezone ||
                null,
...
function mergeCandleRows(previousRows, freshRows) {
    ...
    const safePreviousRows = Array.isArray(previousRows) &&
        ...
        ? previousRows.filter(row => {
            const previousTime = timestampToMs(row?.time);
...
/* =====================================================================
   ...
   ===================================================================== */
function getClosedRows(rows, referenceTimeMs = Date.now()) {
    if (!Array.isArray(rows)) {
        return [];
    }
...
function aggregateClosedCandles(rows, targetMinutes, referenceTimeMs = Date.now()) {
    ...
    const safeReferenceTimeMs = Number.isFinite(referenceTimeMs)
        ? referenceTimeMs
        : Date.now();
    const sourceRows = getClosedRows(rows, safeReferenceTimeMs)
        .map(row => ({
        row,
        timestamp: timestampToMs(row?.time)
    }))
        .filter(item => Number.isFinite(item.timestamp))
        .sort((first, second) => first.timestamp -
        second.timestamp);
    const expectedSourceCount = targetMinutes /
        INTERVAL_MINUTES;
    const targetMs = targetMinutes *
        60000;
    const groups = new Map();
    for (const item of sourceRows) {
        const bucketStart = Math.floor(item.timestamp /
            targetMs) *
            targetMs;
    ...
    for (const [bucketStart, items] of groups.entries()) {
        ...
        for (let index = 0; index < items.length; index++) {
            const expectedTimestamp = bucketStart +
                index * INTERVAL_MS;
...
function buildStrategyContext(rows) {
    ...
    return {
        ...
        recommendedFilters: {
            ...
            volatility: "ATR must be inside a tradable range",
            structure: "Trade with HH/HL or LH/LL structure",
            supportResistance: "Avoid entries directly into nearby barriers",
            candlePattern: "Require rejection, engulfing or breakout confirmation",
            news: "Block conflicting high-impact news",
            riskReward: "Minimum planned reward should justify risk",
            duplicateProtection: "Do not issue repeated signals from the same candle"
        }
    };
}
/* =====================================================================
...
async function main() {
    ...
    const previousOutput = readJsonFile(OUTPUT_PATH, {});
    /*
    ...
    const output = {
        ...
        outputSizeRequested: OUTPUT_SIZE,
        derivedTimeframes: {
            M15: {
                source: "5m",
                grouping: 3,
        ...
        requestPolicy: {
            maximumRequestsPerRun: MAX_REQUESTS_PER_RUN,
            retries: 0,
    ...
    for (let index = 0; index < SYMBOLS.length; index++) {
        ...
        try {
            ...
            const message = error instanceof Error
                ? error.message
                : String(error);
            console.error(`Failed to fetch ${config.symbol}: ${message}`);
            /*
            ...
            if (cached.rows.length > 0) {
                ...
                console.warn(`No cached 5m candles are available for ${config.symbol}.`);
            }
        }
        /*
    ...
    console.log(`Twelve Data requests used: ` +
        `${requestsMade}/${MAX_REQUESTS_PER_RUN}`);
    console.log(`Successful symbols: ` +
